...
TEST_CASE("Brute Force Algorithm") {
  ...
  SUBCASE("Shortest Path A to D") {
    auto path = bruteForce.find_shortest_path(graph, v1, v4);
    REQUIRE(path.size() > 0);
    CHECK(path.size() == 3);
  ...
  SUBCASE("Shortest Path B to D") {
    auto path = bruteForce.find_shortest_path(graph, v2, v4);
    REQUIRE(path.size() > 0);
    CHECK(path.size() == 2);
  ...
  SUBCASE("No Path Test (D to A)") {
    auto path = bruteForce.find_shortest_path(graph, v4, v1);
    CHECK(path.empty());
  }
}
